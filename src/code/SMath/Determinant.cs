...
public static class Determinant
{
    ...
    public static N FromCells<N>(N r1c1, N r1c2, N r1c3,
        ...
        where N : INumberBase<N>
        =>
          r1c1 * r2c2 * r3c3 + r1c2 * r2c3 * r3c1 + r1c3 * r2c1 * r3c2
        - r1c3 * r2c2 * r3c1 - r2c3 * r3c2 * r1c1 - r3c3 * r1c2 * r2c1;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static N FromRows<N>((N C1, N C2, N C3) row1,
                                (N C1, N C2, N C3) row2,
                                (N C1, N C2, N C3) row3)
        where N : INumberBase<N>
        => FromCells(row1.C1, row1.C2, row1.C3,
                     row2.C1, row2.C2, row2.C3,
                     row3.C1, row3.C2, row3.C3);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static N FromColumns<N>((N R1, N R2, N R3) col1,
                                   (N R1, N R2, N R3) col2,
                                   (N R1, N R2, N R3) col3)
        where N : INumberBase<N>
        => FromCells(col1.R1, col2.R1, col3.R1,
