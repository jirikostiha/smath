...
/// It is a sigmoid function rescaled to the interval from minus one to one
/// and equals the hyperbolic tangent of a half of the variable.
/// </summary>
/// <remarks>
...
public class BipolarSigmoid : IMathFunction
{
    ...
    public static N Eval<N>(N x)
        ...
        => (N.CreateChecked(2) / (N.One + N.Exp(-x))) - N.One;

    public static N DerivativeEval<N>(N x)
        where N : IExponentialFunctions<N>
    {
        var value = Eval(x);
        return (N.One - (value * value)) / N.CreateChecked(2);
    }
}
